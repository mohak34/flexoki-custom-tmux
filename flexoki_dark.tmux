...
window_id_style="$(echo "$TMUX_VARS" | grep '@flexoki_dark-tmux_window_id_style' | cut -d" " -f2)"
pane_id_style="$(echo "$TMUX_VARS" | grep '@flexoki_dark-tmux_pane_id_style' | cut -d" " -f2)"
zoom_id_style="$(echo "$TMUX_VARS" | grep '@flexoki_dark-tmux_zoom_id_style' | cut -d" " -f2)"
github_status_enabled="$(echo "$TMUX_VARS" | grep '@flexoki_dark-tmux_github_status' | cut -d" " -f2)"
session_bg_enabled="$(echo "$TMUX_VARS" | grep '@flexoki_dark-tmux_session_bg' | cut -d" " -f2)"
window_center_enabled="$(echo "$TMUX_VARS" | grep '@flexoki_dark-tmux_window_center' | cut -d" " -f2)"
